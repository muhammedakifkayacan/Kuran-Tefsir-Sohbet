...
interface QuranReaderProps {
  ...
  onNavigateTab?: (tab: 'home' | 'quran' | 'sohbet' | 'notes') => void;
}
...
export const QuranReader: React.FC<QuranReaderProps> = ({
  ...
  // Countdown timer for Delete All confirmation modal
  useEffect(() => {
    let timer: any;
    ...
    <div className={`mx-auto animate-fade-in w-full max-w-full overflow-x-hidden transition-all duration-300 ${themeStyles.textMain} ${
      ...
      <div className="sticky top-0 z-40 -mx-3 -mt-3 sm:-mx-5 sm:-mt-5 mb-3 px-3.5 py-2.5 sm:py-3 bg-white/95 dark:bg-stone-900/95 backdrop-blur-md border-b border-stone-200 dark:border-stone-800 flex items-center justify-between gap-2.5 shadow-2xs">
        ...
        <button
          ...
          className="flex items-center gap-1.5 text-stone-800 dark:text-stone-100 hover:text-emerald-700 dark:hover:text-emerald-400 font-bold text-xs bg-stone-100 dark:bg-stone-800 hover:bg-stone-200 dark:hover:bg-stone-700 px-3 py-1.5 rounded-xl border border-stone-200 dark:border-stone-700 active:scale-95 transition-all cursor-pointer shrink-0"
          title="Geri Dön"
          aria-label="Geri Dön"
        >
          <ChevronLeft className="w-4 h-4 text-emerald-700 dark:text-emerald-400" />
          <span>Geri</span>
        </button>
        ...
        >
          ...
          <span className="truncate">{selectedSurah.id}. {selectedSurah.nameTurkish.replace(' Sûresi', '')}</span>
          <span className="text-[11px] text-amber-800 dark:text-amber-400 font-bold shrink-0 bg-amber-200/60 dark:bg-amber-950/80 px-2 py-0.5 rounded-md">
            S.{selectedPage}
          </span>
        </button>
        ...
        <button
          ...
          aria-label="Kur'an Menüsü & Ayarlar"
        >
        ...
        <div className="fixed inset-0 z-[99999] bg-stone-950 text-stone-100 flex flex-col animate-fade-in overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 pb-12 overscroll-contain">
            ...
            <div className="bg-stone-900 border border-stone-800 rounded-2xl p-3.5 space-y-3">
              ...
              <div className="space-y-1.5 pt-2 border-t border-stone-800">
                ...
                <div className="grid grid-cols-4 gap-2">
                  {[
                    { id: 'ivory', name: 'Fildişi', bg: 'bg-[#FCFBF7]', text: 'text-stone-900' },
                    { id: 'mint', name: 'Nane', bg: 'bg-[#F2F7F2]', text: 'text-stone-900' },
                    { id: 'white', name: 'Beyaz', bg: 'bg-[#FFFFFF]', text: 'text-stone-900' },
                    { id: 'dark', name: 'Gece', bg: 'bg-stone-900', text: 'text-stone-100' },
                  ].map((thm) => (
                    <button
                      key={thm.id}
                      type="button"
            ...
            <div className="bg-stone-900 border border-stone-800 rounded-2xl p-3.5 space-y-3">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`flex-1 p-2.5 rounded-xl border text-xs font-bold flex items-center justify-center gap-2 transition-all cursor-pointer ${
                    isPlaying
                ...
                >
                  ...
                  <span>{isPlaying ? 'Durdur' : 'Sayfayı Dinle'}</span>
                </button>
            ...
            <button
              type="button"
              onClick={() => setIsMobileBurgerMenuOpen(false)}
              className="w-full py-3 px-4 rounded-xl bg-amber-500 hover:bg-amber-600 text-stone-950 font-black text-sm transition-all shadow-lg cursor-pointer mt-2"
            >
              Okumaya Devam Et
            </button>
          </div>
          ...
          {viewMode === 'mushaf' && (
            <div ref={mushafContainerRef} className="max-w-4xl mx-auto w-full space-y-4">
              ...
              >
                ...
                <div className={`flex items-center justify-between border-b border-[#D4AF37]/15 pb-3 mb-5 pr-16 sm:pr-24 text-[11px] font-bold uppercase tracking-wide transition-colors ${themeStyles.textMuted}`}>
      ...
      {viewMode === 'detailed' && (
        <div className="space-y-4">
          ...
          {(!isFullScreen || areOverlaysVisible) && (
            <div className="hidden sm:block bg-white dark:bg-stone-900 rounded-3xl p-4 border border-stone-200/80 dark:border-stone-800 shadow-sm space-y-3">
      ...
      {/* TÜM İPLERİ SİLME SAYAÇLI UYARI POPUP MODALI */}
      {isDeleteAllConfirmOpen && (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-stone-950/75 backdrop-blur-md animate-fade-in">
          <motion.div
            initial={{ opacity: 0, scale: 0.9, y: 10 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-3xl p-6 max-w-md w-full shadow-2xl space-y-5 text-stone-900 dark:text-stone-100 relative overflow-hidden text-center"
          >
            <div className="w-14 h-14 rounded-full bg-rose-100 dark:bg-rose-950 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-900 mx-auto flex items-center justify-center text-2xl font-black shadow-inner">
              ⚠️
            </div>

            <div className="space-y-2">
              <h3 className="text-lg font-bold text-stone-900 dark:text-stone-100">
                Tüm İp Ayraçları Silme Onayı
              </h3>
              <p className="text-xs text-stone-600 dark:text-stone-300 font-medium leading-relaxed">
                Kaydetmiş olduğunuz <strong className="text-rose-600 dark:text-rose-400 font-bold">{ribbons.length} adet</strong> ip ayraç ve sayfa işareti kalıcı olarak silinecektir. Yanlışlıkla silinmeyi önlemek için geri sayım tamamlanmalıdır.
              </p>
            </div>

            {/* Countdown Badge / Timer Circle */}
            <div className="py-2 flex flex-col items-center justify-center">
              <div className="w-16 h-16 rounded-full border-4 border-rose-200 dark:border-rose-900/50 border-t-rose-600 dark:border-t-rose-500 animate-spin flex items-center justify-center relative">
                <span className="text-xl font-black text-rose-600 dark:text-rose-400 !animate-none" style={{ animation: 'none' }}>
                  {deleteCountdown > 0 ? deleteCountdown : '✓'}
                </span>
              </div>
              <span className="text-[11px] font-bold text-stone-500 dark:text-stone-400 mt-2">
                {deleteCountdown > 0
                  ? `Lütfen ${deleteCountdown} saniye bekleyin...`
                  : 'Silme butonu aktif hale geldi!'}
              </span>
            </div>
            ...
            <div className="flex items-center gap-3 pt-2">
              <button
                ...
                className="flex-1 py-3 rounded-2xl bg-stone-100 dark:bg-stone-800 hover:bg-stone-200 dark:hover:bg-stone-700 text-stone-800 dark:text-stone-200 font-bold text-xs transition-colors cursor-pointer"
              >
                İptal Et
              </button>
              ...
              <button
                ...
                className={`flex-1 py-3 rounded-2xl font-bold text-xs transition-all shadow-md flex items-center justify-center gap-1.5 ${
                  deleteCountdown > 0
                    ? 'bg-stone-200 dark:bg-stone-800 text-stone-400 dark:text-stone-600 cursor-not-allowed border border-stone-300 dark:border-stone-700'
                    : 'bg-rose-600 hover:bg-rose-700 text-white shadow-rose-600/30 active:scale-95 cursor-pointer'
                }`}
              >
                {deleteCountdown > 0 ? (
                  <span>Bekleyin ({deleteCountdown}s)</span>
                ) : (
                  <span>Evet, Tüm İpleri Sil</span>
                )}
              </button>
            </div>
          </motion.div>
      ...
      {/* FLOATING APPLE-LEVEL PAGE TURN BAR (SADECE KURAN METNİ VE YALIN SAYFA ÇEVİRME) */}
      {!selectedMushafAyah && (
        <div className="fixed bottom-5 left-1/2 -translate-x-1/2 z-30 bg-stone-900/90 dark:bg-stone-900/95 text-stone-100 backdrop-blur-xl px-4 py-2.5 rounded-full border border-stone-700/80 shadow-2xl flex items-center gap-2.5 animate-fade-in transition-all">
          {/* Önceki Sayfa */}
          <button
            type="button"
            disabled={!hasPrevPage}
            ...
            className="p-1.5 rounded-full hover:bg-stone-800 text-stone-300 hover:text-white disabled:opacity-20 transition-all active:scale-90 cursor-pointer flex items-center gap-1 text-xs font-bold"
            title="Önceki Sayfa"
          >
            <ChevronLeft className="w-4 h-4 text-amber-400" />
          ...
          <button
            ...
            className="px-3.5 py-1 rounded-full bg-stone-800 hover:bg-stone-750 text-amber-300 text-xs font-black border border-amber-500/30 flex items-center gap-1.5 cursor-pointer active:scale-95 transition-all shadow-2xs"
            title="Sayfaya Git"
          >
            <BookOpen className="w-3.5 h-3.5 text-amber-400" />
            <span>Sayfa {selectedPage} / 604</span>
          </button>
